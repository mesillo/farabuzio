...
let createTempFile = ( config ) => {
	let filename = config.tempDirectory +"/"+ ( Math.abs( Math.round( Math.random() * 1000000 ) ) ) + ".py";
	let template = __dirname + "/lib/templates/handler.py";
	let replaceCommand = `cat ${template} | sed 's/=lambda=name=placeholder=/${config.functionName}/g' > ${filename}`;
	let deployLambdaCommand = `awslocal lambda create-function --function-name=${config.functionName} --runtime=python2.7 --region=fakeRegion --role=fakeRole --handler=${filename}.hendler file://${filename}`; //TODO: check the runtime (python3???)...

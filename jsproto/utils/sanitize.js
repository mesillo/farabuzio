...
let objectToSanitize = {
	"results": [
	  ...
	  {
		  "BadString": "<script>document.getElementById(\"demo\").innerHTML = \"Hello JavaScript!\";</script>",
	  }
	],
	"status": "OK"
  };
...
class ObjectUtils {
	...
	static XSSsanitizeString( unsafeString ) {
		return unsafeString.replace( /\W/g, ( match ) => {
			//console.log( match + " ==> " + ( sanitizeDefines.toReplace[ match ] || match ) );
			return ( sanitizeDefines.toReplace[ match ] || match );
		} );
	}
}

ObjectUtils.doForAllStrings( objectToSanitize, ObjectUtils.XSSsanitizeString );
console.dir( objectToSanitize, { showHidden: false, depth: null, colors: true } );
